...
import { revokeAdminIdentity } from "@/features/compliance/api/client";
import { isWalletAddress } from "@/shared/lib/formatters";
import { Alert } from "@/shared/ui/Alert";
import { Button } from "@/shared/ui/Button";
import { useMessages } from "@/shared/i18n/LocaleProvider";

/** CO-S04 — Revoke Eligibility */
export function CoRevokeScreen() {
  const m = useMessages();
  ...
  return (
    <div className="panel" data-screen-id="CO-S04">
      <h1>Revoke eligibility</h1>
      <p className="muted">
        Removes investor eligibility on-chain. Transfers to/from this wallet will fail at execution.
      </p>
      ...
      <Button disabled={loading} onClick={() => void revoke()}>
        {m.workspace.compliance.nav.revoke}
      </Button>
    </div>
  );
}
